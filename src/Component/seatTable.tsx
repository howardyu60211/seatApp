...
export class SeatTable extends Component<NonNullable<unknown>, SeatTableState> {
    ...
    changeProps = (i: number, status: seatStatus, text = "") => {
    ...
    phaseExcel = (students: string[]) => {
        ...
        for (const randomIndex of shuffledSeatList) {
            if (i < students.length && this.state.seatStatusList[randomIndex] === seatStatus.ava) {
                this.changeProps(randomIndex, seatStatus.occ, students[i])
                console.log("set", randomIndex, "to", students[i])
                i++
            } else {
                this.changeProps(randomIndex, seatStatus.emp)
    ...
    render() {
        ...
        for (let i = 0; i < this.state.c; i++) {
            for (let j = 0; j < this.state.r; j++) {
                ...
                seat.push(<SeatButton key={String(cur)} id={String(cur)} draggable="true" editor={this.changeProps}
                ...
                <div className="mt-4 w-full px-6 flex flex-wrap">
                    ...
                    <div className="w-full max-w-full pb-1 pl-0">
                        <div className="flex flex-wrap -mx-3 pb-2">
                            {/* text */}
                            <div className="flex flex-col flex-none w-1/2 max-w-full px-3">
                            ...
                            {/* Button */}
                            <div className="flex flex-row-reverse w-1/2 max-w-full px-3 text-right">
                                <button
                                ...
                                <button onClick={this.exportExcel}
                                        className="select-none h-11 px-4 mx-1 mb-0 font-bold text-center uppercase align-middle transition-all bg-transparent border border-solid rounded-lg shadow-none cursor-pointer leading-pro ease-soft-in text-xs duration-100 bg-150 active:opacity-85 hover:scale-102 tracking-tight-soft bg-x-25 border-opacity-0 hover:border-opacity-100 border-fuchsia-500 text-fuchsia-500">匯出座位
                                </button>
                            </div>
                        </div>
                    </div>
                    ...
                    <StatusBar
                        blueNum={this.state.seatStatusList.filter((x) => {return x == seatStatus.occ}).length}
                        greenNum={this.state.seatStatusList.filter((x) => {return x == seatStatus.ava}).length}
                        redNum={this.state.seatStatusList.filter((x) => {return x == seatStatus.emp}).length}
                            >
                            </StatusBar>
                </div>
            </>
