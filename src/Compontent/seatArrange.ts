import { createSeat, seatStatus, shuffle, type Seat, type SeatStudent } from "./seatModel";
...
export type ArrangeResult =
    { ok: true; seats: Seat[]; filledOrder: number[] } | { ok: false; error: string };
...
const isPinnedSeat = (seat: Seat) => seat.status === seatStatus.occ && seat.pinned;

const isMovableSeat = (seat: Seat) => seat.status === seatStatus.occ && !seat.pinned;

const collectMovableStudents = (seats: readonly Seat[]): SeatStudent[] =>
    seats.filter(isMovableSeat).map((seat) => ({ name: seat.name, tag: seat.tag }));
...
const sortStudentsByNumber = (students: readonly SeatStudent[], order: NumericOrder) =>
...
const buildSeatGroups = (rowCount: number, colCount: number, grouping: NumericGrouping) => {
    const groups: number[][] = [];
...
const assignAlternatingByTag = (
    ...
    for (const seatIndex of orderedTargets) {
        ...
        const preferred = remaining.filter((bucket) => !neighborTags.has(bucket[0].tag.trim()));
        const pool = preferred.length > 0 ? preferred : remaining;
        const picked = pool.reduce((best, bucket) => (bucket.length > best.length ? bucket : best));
    ...
    return {
        assignments,
        filledOrder: orderedTargets.filter((seatIndex) => assignments.has(seatIndex)),
    };
};
...
export const arrangeSeats = (
    ...
    const nextSeats = seats.map((seat, index) => {
        ...
        if (options.mode === "exam") {
            return createSeat(disabledIndexes.has(index) ? seatStatus.emp : seatStatus.ava);
        }

        return createSeat(seat.status === seatStatus.emp ? seatStatus.emp : seatStatus.ava);
    });
